...
namespace OpenEphys.AcquisitionBoard
{
    ...
    /// internal device manager. This is restricted to onix-specific devices which use <see cref="AcquisitionBoardContextTask"/>
    /// </summary>
    /// <remarks>
    /// ONI devices usually require a specific sequence of configuration and parameterization steps before
    /// they can be interacted with. The <see cref="AcquisitionBoardSingleDeviceFactory"/> provides a modular abstraction
    /// allowing flexible assembly and sequencing of of all device-specific configuration code.
    /// </remarks>
    [WorkflowElementCategory(ElementCategory.Sink)]
    [Combinator]
    public abstract class AcquisitionBoardSingleDeviceFactory : IDeviceCollection, IDeviceConfiguration, IAddressableDevice
    {
        ...
        internal AcquisitionBoardSingleDeviceFactory(Type deviceType)
        {
        ...
        public abstract IObservable<AcquisitionBoardContextTask> Process(IObservable<AcquisitionBoardContextTask> context);
        ...
        IObservable<TContext> IDeviceConfiguration.Process<TContext>(IObservable<TContext> context)
        {
            ...
            if (!typeof(TContext).IsAssignableFrom(typeof(AcquisitionBoardContextTask)) && !typeof(AcquisitionBoardContextTask).IsAssignableFrom(typeof(TContext)))
            {
                throw new InvalidCastException($"Cannot cast {typeof(TContext).Name} to {nameof(AcquisitionBoardContextTask)}");
            }
            var ctx = context.Cast<AcquisitionBoardContextTask>();
            var result = Process(ctx);
